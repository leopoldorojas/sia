class SalesController < ApplicationController
  ...
  def search
    ...
    @share_operations = @share_operations.since(params[:start_date]) if params[:start_date].present?
    @share_operations = @share_operations.until(params[:end_date]) if params[:end_date].present?
    @share_operations = @share_operations.receipt_like(params[:receipt]) if params[:receipt].present?
    @share_operations = @share_operations.share_holder_is(params[:share_holder_id]) if params[:share_holder_id].present?
    respond_to :json
  end
