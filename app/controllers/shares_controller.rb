class SharesController < ApplicationController
  ...
  def search
    @shares = Share.all
    @shares = @shares.where("shares.identifier >= (?)", "#{params[:first_identifier]}" ) if params[:first_identifier].present?
    @shares = @shares.where("shares.identifier <= (?)", "#{params[:last_identifier]}" ) if params[:last_identifier].present?
    ...
    @shares = @shares.where.not(share_holder_id: [0, nil]) if params[:sold_or_not] == "sold"
    @shares = @shares.where(share_holder_id: [0, nil]) if params[:sold_or_not] == "no_sold"
    respond_to :json
  end
  ...
  private
    ...
    def share_params
      params.require(:share).permit(:identifier, :share_type, :share_holder_id, :share_issue_id)
    end
end
